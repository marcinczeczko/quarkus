...
public class S3Resource {
    ...
    public CompletionStage<String> testAsyncS3() {
        LOG.info("Testing Async S3 client with bucket: " + SYNC_BUCKET);
        String keyValue = UUID.randomUUID().toString();

        return S3Utils.createBucketAsync(s3AsyncClient, SYNC_BUCKET)
                .thenCompose(bucket -> s3AsyncClient.putObject(S3Utils.createPutRequest(SYNC_BUCKET, keyValue),
                        AsyncRequestBody.fromString(SAMPLE_S3_OBJECT)))
                .thenCompose(resp -> s3AsyncClient.getObject(S3Utils.createGetRequest(SYNC_BUCKET, keyValue),
    ...
    public String testBlockingS3() {
        LOG.info("Testing S3 Blocking client with bucket: " + ASYNC_BUCKET);

        String keyValue = UUID.randomUUID().toString();
        String result = null;

        try {
            if (S3Utils.createBucket(s3Client, ASYNC_BUCKET)) {
                if (s3Client.putObject(S3Utils.createPutRequest(ASYNC_BUCKET, keyValue),
                        RequestBody.fromString(SAMPLE_S3_OBJECT)) != null) {
                    ByteArrayOutputStream baos = new ByteArrayOutputStream();

                    GetObjectResponse object = s3Client.getObject(S3Utils.createGetRequest(ASYNC_BUCKET, keyValue),

...
The solution is located in the `s3-quickstart` directory.
...
mvn io.quarkus:quarkus-maven-plugin:{quarkus-version}:create \
    ...
    -DprojectArtifactId=s3-quickstart \
...
In the second step let's create a bean that will represent a file in a S3 bucket as follows:
...
Now create a `org.acme.s3.CommonResource` that will consist of helper methods that prepare S3 request objects for reading and adding items to the bucket.
...
public class S3SyncClientResource extends CommonResource {
    ...
    public List<FileObject> listFiles() {
        ...
        return s3.listObjects(listRequest).contents().stream().sorted(Comparator.comparing(S3Object::lastModified).reversed())
                .map(this::toFileItem).collect(Collectors.toList());
    }

    private FileObject toFileItem(S3Object s3Object) {
...
TBD: The implementation is pretty straightforward and you just need to define your endpoints using the JAX-RS annotations and use the `FruitSyncService` to list/add new fruits.
...
[source,properties]
----
quarkus.amazon.sync-client.s3.type=apache
----
...
[source,properties]
----
quarkus.amazon.s3.endpoint-override=http://localhost:8008

quarkus.amazon.aws.s3.region=us-east-1
quarkus.amazon.aws.s3.credentials.type=static
quarkus.amazon.aws.s3.credentials.static-provider.access-key-id=test-key
quarkus.amazon.aws.s3.credentials.static-provider.secret-access-key=test-secret
----

- `quarkus.amazon.aws.s3.region` - It's required by the client, but since you're using a local S3 instance you can pick any valid AWS region.
- `quarkus.amazon.aws.s3.credentials.type` - Set `static` credentials provider with any values for `access-key-id` and `secret-access-key`
- `quarkus.amazon.s3.endpoint-override` - Override the S3 client to use a local instance instead of an AWS service

If you want to work with an AWS account, you'd need to set it with:
[source,properties]
----
quarkus.amazon.aws.s3.region=<YOUR_REGION>
quarkus.amazon.aws.s3.credentials.type=default
----

- `quarkus.amazon.aws.s3.region` you should set it to the region where your S3 bucket was created,
- `quarkus.amazon.aws.s3.credentials.type` - use the `default` credentials provider chain that looks for credentials in this order:
...
It can be run with `java -jar target/s3-quickstart-1.0-SNAPSHOT-runner.jar`.
...
public class S3AsyncClientResource extends CommonResource {
    ...
    private List<FileObject> toFileItems(ListObjectsResponse objects) {
        return objects.contents().stream()
                ...
                .map(obj -> new FileObject(obj.key(), obj.size()))
...
=== S3 service configuration reference

include::{generated-dir}/config/quarkus-s3.adoc[opts=optional, leveloffset=+1]

...
quarkus.amazon.s3.region=us-east-1
quarkus.amazon.s3.credentials.type=static
quarkus.amazon.s3.credentials.static-provider.access-key-id=test-key
quarkus.amazon.s3.credentials.static-provider.secret-access-key=test-secret

bucket.name=quarkus.s3.quickstart
----

- `quarkus.amazon.s3.region` - It's required by the client, but since you're using a local S3 instance you can pick any valid AWS region.
- `quarkus.amazon.s3.credentials.type` - Set `static` credentials provider with any values for `access-key-id` and `secret-access-key`
...
quarkus.amazon.s3.region=<YOUR_REGION>
quarkus.amazon.s3.credentials.type=default
----

- `bucket.name` - name of the S3 bucket on your AWS account.
- `quarkus.amazon.s3.region` you should set it to the region where your S3 bucket was created,
- `quarkus.amazon.s3.credentials.type` - use the `default` credentials provider chain that looks for credentials in this order:

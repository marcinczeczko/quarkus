...
quarkus.amazon.dynamodb.region=eu-central-1
quarkus.amazon.dynamodb.credentials.type=static
quarkus.amazon.dynamodb.credentials.static-provider.access-key-id=test-key
quarkus.amazon.dynamodb.credentials.static-provider.secret-access-key=test-secret
----

- `quarkus.amazon.dynamodb.region` - It's required by the client, but since you're using a local DynamoDB instance you can pick any valid AWS region.
- `quarkus.amazon.dynamodb.credentials.type` - Set `static` credentials provider with any values for `access-key-id` and `secret-access-key`
...
[source,properties]
----
quarkus.amazon.dynamodb.region=<YOUR_REGION>
quarkus.amazon.dynamodb.credentials.type=default
----

- `quarkus.amazon.dynamodb.region` you should set it to the region where you provisioned the DynamoDB table,
- `quarkus.amazon.dynamodb.credentials.type` - use the `default` credentials provider chain that looks for credentials in this order:
